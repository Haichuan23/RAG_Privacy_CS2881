...
def main(my_args, llm_args, ric_args, knn_args, training_args, data_args):
    ...
    if my_args.task == "debug":
        ...
        # For local models, we need hf_ckpt and is_chat_model
        assert llm_args.hf_ckpt is not None, "hf_ckpt must be provided for local models"
        assert llm_args.is_chat_model is not None, (
            "is_chat_model must be specified for local models"
        )
        ...
        for dict_item in tqdm(js):
            ...
            with open(file_to_save, "w") as f:
                json.dump(
                    {"lm_output": lm_output, "retrieved_docs_str": retrieved_docs_str},
                    f,
                    indent=4,
                )
        ...
        for model_name in tqdm(os.listdir(data_args.eval_input_dir)):
            if os.path.exists(
                os.path.join(data_args.eval_output_dir, model_name + ".json")
            ):
                continue
            json_files = [
                os.path.join(data_args.eval_input_dir, model_name, f)
            ...
            evaluator = Evaluator(
                predictions_str=predictions_str, references_str=references_str
            )
            metrics = evaluator.compute_metrics()
            with open(
                os.path.join(data_args.eval_output_dir, model_name + ".json"), "w"
            ) as f:
